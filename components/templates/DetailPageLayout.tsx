...
export function DetailPageLayout({
  ...
  return (
    <article className="pb-4">
      <Link
        href={backHref}
        className="mb-8 inline-flex items-center gap-2 text-sm text-zinc-400 transition-colors hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]/40 rounded-sm"
      >
        ← {backLabel}
      </Link>
      {children}
    ...
    <ol className="relative space-y-6 border-l border-white/10 pl-6">
      {events.map((item, index) => (
        <li key={index} className="relative">
          <span className="absolute -left-[1.65rem] top-1.5 h-2.5 w-2.5 rounded-full bg-[var(--accent)] ring-4 ring-zinc-950" />
          <p className="text-xs font-medium uppercase tracking-wide text-[var(--accent-secondary)]">
            {item.date}
          </p>
